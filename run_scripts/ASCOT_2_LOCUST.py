...
def ASCOT_2_LOCUST(path_LOCUST='',path_ASCOT='',beam_depo_GC=True,filename_ASCOT_equilibrium='ASCOT_GEQDSK',GEQDSKFIX=0,species_numbers=[1],wall_type='2D',tag=''):
    ...
    try:
        temperature_array,number_density_array,temperature_e,number_density_e,beam_deposition,wall=run_scripts.utils.read_inputs_ASCOT(input_path=path_ASCOT,beam_depo_GC=beam_depo_GC,species_numbers=species_numbers,wall_type=wall_type)
    ...
    try: #run dump_inputs_LOCUST without ion temperature - dump these separately due to possible multiple species
        ...
        for number_density in number_density_array:
            number_density.dump_data(data_format='LOCUST',filename='profile_ni{}.dat'.format(number_density.properties['species_number']))
