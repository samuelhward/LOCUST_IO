...
try:
    ...
    raise ImportError("ERROR: ASCOT_IO/run_scripts/utils.py could not be imported!\nreturning\n")
    sys.exit(1)  

try:
    import support
except:
    raise ImportError("ERROR: ASCOT_IO/support.py could not be imported!\nreturning\n") 
    sys.exit(1)
try:
    from constants import *
except:
    raise ImportError("ERROR: ASCOT_IO/constants.py could not be imported!\nreturning\n") 
    sys.exit(1)
try:
    from settings import *
except:
    raise ImportError("ERROR: ASCOT_IO/settings.py could not be imported!\nreturning\n") 
    sys.exit(1)
...
def TRANSP_2_ASCOT(run_ID,shot_number,input_path='',output_path='',beam_depo_GC=True,beam_depo_weighted=True,tag=''):
    """
    ...
    #XXX NEED ROTATION ETC here

    try:
        run_scripts.utils.dump_inputs_ASCOT(temperature_i=temperature_i,temperature_e=temperature_e,density_e=density_e,equilibrium=equilibrium,beam_deposition=beam_deposition,wall=wall,beam_depo_GC=beam_depo_GC,beam_depo_weighted=beam_depo_weighted,BCHECK=False,wall_type='2D',tag=tag)
    except:
...
if __name__=='__main__':
    ...
    #convert 
    TRANSP_2_ASCOT(run_ID=args.run_ID,shot_number=args.shot_number,input_path=args.input_path,output_path=args.output_path,beam_depo_GC=args.beam_depo_GC,beam_depo_weighted=args.beam_depo_weighted,tag=args.tag)
