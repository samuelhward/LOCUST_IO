...
for r_sample_point,z_sample_point in zip(r_sample_points,z_sample_points):
    ...
    for radius,LOCUST_file,ASCOT_file,run_ID,colour in zip(radii,LOCUST_files,ASCOT_files,run_IDs,colours):
        ...
        TRANSP_mesh=TRANSP_dfn.dfn_plot(axes=axes,limiters=wall,LCFS=equi,ax=ax1,fig=fig,vminmax=vminmax)
        ASCOT_mesh=ASCOT_dfn.dfn_plot(axes=axes,limiters=wall,LCFS=equi,ax=ax2,fig=fig,vminmax=vminmax)
        ...
        TRANSP_mesh=TRANSP_dfn.dfn_plot(axes=axes,ax=ax7,fig=fig,vminmax=vminmax) #plotting with slices and specifying the point plots from the non-integrated distribution function

        axes=[slice(None),slice(None),index_r_ascot,index_z_ascot]
        ASCOT_mesh=ASCOT_dfn.dfn_plot(axes=axes,ax=ax8,fig=fig,vminmax=vminmax)
