...
parameters__phases_uppers=[
    np.linspace(0,120,7)[:-1][1]+np.array([84.59014209730469, 97.97671623514145, 157.45157877171604, 244.88956133500577, 309.2524113727721, 295.8771305794595, 236.3980603134114, 148.970331391124])/3.+30-3.3,
    np.linspace(0,90,7)[:-1][4]+np.array([-225.94568453777023, -211.08243707342373, -150.69617805444034, -64.54854169694258, 0.8932135128582925, -13.98452459543561, -74.36047160900125, -160.5089086556394])/4.+30-3.3
    ]

parameters__phases_middles=[
    np.full(len(parameters__phases_uppers[0]),np.linspace(0,120,7)[:-1][1])+26.7,
    np.full(len(parameters__phases_uppers[1]),np.linspace(0,90,7)[:-1][4])+26.7
]

parameters__phases_lowers = [
    np.linspace(0,120,7)[:-1][1]+np.array([75.0, 167.8797372399381, 243.0224383846416, 269.33915048282586, 203.18397667399648, 110.31083349576636, 35.175600056809, 8.854857448075652])/3.+30-3.3,
    np.linspace(0,90,7)[:-1][4]+np.array([30.0, 120.09213962358345, 191.14556644140822, 219.08551613857986, 159.15659320240238, 69.06617432444777, -1.994981193681455, -29.924349163320883])/4.+30-3.3
    ]
