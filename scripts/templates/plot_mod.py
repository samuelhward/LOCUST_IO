...
def read_locust_io_obj(filename,classtype='eq',*args,**kwargs):
    ...
    for i in range(16): data_format_dispatch[f'rund_{i+1}'] = 'LOCUST'
    ...
    for i in range(16): classes_dispatch[f'rund_{i+1}'] = Rundata
...
def get_io_files(batch_data=None,classtype='eq',yield_filenames=False,return_lists=False,*args,**kwargs):
    ...
    for i in range(16): file_extension_dispatch[f'rund_{i+1}'] = f'rundata*_{i+1}'
    ...
    for i in range(16): io_type_dispatch[f'rund_{i+1}'] = 'output'
