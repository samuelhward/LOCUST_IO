...
todo:
    ...
    XXX for NEMO develop latest need function to edit xml file https://stackabuse.com/reading-and-writing-xml-files-in-python/
...
class RMP_study_run(run_scripts.workflow.Workflow):
    ...
    def __init__(self,
        ...
        self.add_command(command_name='plot_kinetic',command_function=self.plot_kinetic_profiles,position=3)
    ...
    def plot_kinetic_profiles(self,*args,**kwargs):
        ...
        for species_name,species_AZ in self.args['table_species_AZ'].items(): #loop through all non-electronic ion species and set equal temperature
            try:
                kinetic_profiles.append(Number_Density(ID='',data_format='LOCUST',species=species_name,filename=self.args['LOCUST_run__dir_input'] / 'density_{}'.format(species_name)))
            except:
                pass
            kinetic_profiles.append(Number_Density(ID='',data_format='LOCUST',species=species_name,filename=self.args['LOCUST_run__dir_input'] / 'profile_ne.dat'))
 
        import matplotlib.pyplot as plt
        fig,ax=plt.subplots(1)
        for kinetic_profile in kinetic_profiles:
            kinetic_profile.plot(label=kinetic_profile.properties['species'],ax=ax,fig=fig)
        plt.show()
