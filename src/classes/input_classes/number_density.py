    ...
    if properties['species']=='electrons':
        ...
        if 'Z' in properties:
            ...
            if species_number is None:
                print("ERROR: read_number_density_IDS could not find requested species in IDS (shot - {shot}, run - {run}, Z - {Z})!\nreturning\n!".format(shot=shot,run=run,Z=int(properties['Z'])))
                return
        else:
            species_number=0
...
class Number_Density(classes.base_input.LOCUST_input):
    ...
    def plot(self,axis='flux_pol_norm',colmap=cmap_default,ax=False,fig=False):
        ...
        ax.plot(self[axis],self['n'],color=colmap(np.random.uniform()))
        ax.set_xlabel(axis)
