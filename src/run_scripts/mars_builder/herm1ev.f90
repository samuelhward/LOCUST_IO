...
!
      ...
      integer :: nxm, ii
      real( gpu ) :: zxget, zxtol

      real( gpu ) :: xget                         ! target point
      real( gpu ) :: x(nx)                        ! indep. coordinate, strict ascending
...
!
      real( gpu ) :: xparam                       ! (xget-x(i))/(x(i+1)-x(i))
!
!  grid spacing
!
      real( gpu ) :: hx                           ! hx = x(i+1)-x(i)
!
!  inverse grid spacing:
!
      real( gpu ) :: hxi                          ! 1/hx = 1/(x(i+1)-x(i))
!
...
!
 ...
 1001       format(' ?herm1ev:  xget=',1pe11.4,' out of range ',1pe11.4,' to ',1pe11.4)
         else
            if((xget.lt.x(1)-0.5*zxtol).or.(xget.gt.x(nx)+0.5*zxtol)) &
...
!
      subroutine herm1fcn(ict,ivec,ivecd,fval,ii,xparam,hx,hxi,fin,nx)
!
!  input:
!
      use prec_mod
...
!
      real( gpu ) :: fin(0:1,nx)                  ! Hermite dataset
!
!  output:
!
      real( gpu ) :: fval(ivecd,*)                ! interpolation results
!
...
!
      ...
      real( gpu ) :: xp, xpi, xp2, xpi2, ax, axbar, bx, bxbar, axp, axbarp, bxp, bxbarp
