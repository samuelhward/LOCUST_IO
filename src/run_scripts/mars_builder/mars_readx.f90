...
#else
      character( len=11 ),                                                    &
                    dimension(2)  :: TAIL = ['_resp_upper','_resp_lower']
#endif
...
!     ITER phase adjustment settings:
      ...
      real( gpu )                 :: R_p, Z_p, d_p, det, ds, dchi, JAC
...
!     Load data:
      ...
      write(io(1),*) ':mars_read : FILE : ', file//TAIL(j)

      open ( unit=lun, file=file//TAIL(j), form='formatted' )
#else
...
!                          Only fill if s<=1 and PSIn<=1 or s>0 and PSIn>0....
                           ...
                           if( (s_ <= 1.0_gpu .and. f(1) <= 1.0_gpu) .or.     &
                               ...
                               if( cells(j,k,7)==HUGE(0.0_gpu) )then
...
#if defined (FXXYY)
      allocate( B( nR_f, nZ_f, 7, 7, 6 )  )
      B       ( 1:nR_f,1:nZ_f,1:7,1:7,1:6 ) = 0.0_gpu
#else
      allocate( B( nR_f, nZ_f, 1, 1, 6 )  )
      B       ( 1:nR_f,1:nZ_f,1,  1,  1:6 ) = 0.0_gpu
#endif
      ...
      do iR=1,nR_f
         do iZ=1,nZ_f
            do iter=1,NMAX
               ...
               chi_ = cells_h(iR,iZ,2)+cells_h(iR,iZ,6)
               
               if( chi_ < 0.0_gpu    )chi_ = chi_ + 2.0_gpu*pi
               if( chi_ > 2.0_gpu*pi )chi_ = chi_ - 2.0_gpu*pi
...
!      write(5,*) NL+1, nR_f, nZ_f
!      write(5,*) R0F_, R1F_, Z0F_, Z1F_
!      write(5,*) R_mag_, Z_mag_
!      write(5,*) tht
!      write(5,*) rho_2
!      do iR=jR0,jR1
!         do iZ=jZ0,jZ1
!            do i =1,6
!#if defined (FXXYY)
!               write(5,*) B(1:nR_f,1:nZ_f,iR,iZ,i)
!#else
!               write(5,*) B(1:nR_f,1:nZ_f,1, 1, i)
!#endif
!            enddo
!         enddo
!      enddo

!      write(5,*) cells_h(1:nR_f,1:nZ_f,1:7)

!      write(41,*) nR_f, nZ_f
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 1)
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 2)
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 3)
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 4)
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 5)
!      write(41,*) B(1:nR_f,1:nZ_f,1, 1, 6)
!      write(41,*) R0F_, R1F_, Z0F_, Z1F_
...
print*, R0F_+real(50,gpu)*(R1F_-R0F_)/real(nR_f-1,gpu), Z0F_+real(100,gpu)*(Z1F_-Z0F_)/real(nZ_f-1,gpu)
print*, BR0(51,101,1),BZ0(51,101,1),BT0(51,101,1)
print*,  B(51,101,1, 1, 1),cos(PHI_(1)*nmde),                 &
         B(51,101,1, 1, 2),sin(PHI_(1)*nmde), PHI_(1)*nmde
